...
public class Mecanica {
	...
	public void GeneradorTrebol() {
		for (int i = 0; i < ta.getTableroJuego().length; i++) {
			for (int j = 0; j < ta.getTableroJuego().length; j++) {

				if (ta.getTableroJuego()[i][j] instanceof PlantaGirasol) {
					if (contador % 4 == 0) {
						contadorTrebol = recursos.getTrebol();
						recursos.setTrebol(contadorTrebol += 1);
	...
	public void seleccionUsuario() {
		...
		System.out.println(
				"RECURSOS DISPONIBLES: " + "Trebol: " + recursos.getTrebol() + ", Agua: " + recursos.getAgua());
		System.out.println("-------------------------------------------------------------");
	...
	public void menuSeleccion() {
		...
		System.out.println(
				"[2] -> Planta Girasol; Caractersticas: Coste de Trebol 3, Vida 30, Ataque 2, Defensa 5, Permite obtener un trebol cada 4 turnos");
		System.out.println(
				"[3] -> Planta Carnivora; Caractersticas: Coste de Trebol 6, Vida 50, Mata a un enemigo cada 4 turnos, Defensa 9");
		System.out.println(
				"[4] -> Planta Guisante; Caractersticas: Coste de Trebol 4, Vida 35, Ataque 3, Defensa 8, Puede atacar a distancia");
		...
		System.out.println("Que planta desea agregar? \nUsted cuenta con los siguientes recursos: " + "Trebol: "
				+ recursos.getTrebol() + ", Agua: " + recursos.getAgua());
		...
		case 1:
			...
			System.out.println(
					"Por favor, ingrese la posicion del tablero de acuerdo a las siguientes indicaciones: Posicion Y / Posicion X");
		...
		case 5:
			...
			ta.pintarTablero();
			ta.moverMonstruo();
			System.out.println("Registro de actividad en la ronda: " + (contador += 1));
			ta.buscarPelea();
			ta.limpiarMuertos();
			if (ta.buscarGanador() == false) {
				...
				System.out.println("Perdiste, Gracias por jugar! Hasta pronto");			}
